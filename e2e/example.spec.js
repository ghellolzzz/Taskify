    ...
    test('should allow a user to create a time entry', async ({ page }) => {
        ...
        await page.waitForFunction(
            ({ taskId, minutes }) => {
                ...
                return sel && String(/** @type {HTMLSelectElement} */ (sel).value) === String(taskId) && min && /** @type {HTMLInputElement} */ (min).value === minutes;
            },
            { taskId, minutes: '45' },
            { timeout: 5000 }
        );
        ...
        const postPromise = page.waitForResponse((resp) => {
            ...
            return url.includes('/api/time-entries') && method === 'POST' && status >= 200 && status < 300;
        });
        await page.evaluate(() => /** @type {HTMLElement | null} */ (document.querySelector('.form-section button.btn-success'))?.click());
        await postPromise;
        await page.waitForLoadState('networkidle');
