...
async function createTaskViaAPI(page, taskData) {
  ...
  // Returns a due date and day number that are always in the CURRENT month so the task is visible when calendar opens.
  ...
  // This keeps tasks on the visible calendar and lets the Priority Suggestions model use current date.
  async function createTestTasks(page) {
    ...
    // Overdue (relative to today) – so Priority Suggestions panel has overdue items by current date
    const twoDaysAgo = new Date(now);
    twoDaysAgo.setDate(twoDaysAgo.getDate() - 2);
    const yesterday = new Date(now);
    yesterday.setDate(yesterday.getDate() - 1);
    await postTask('Overdue 2 days ago', formatDate(twoDaysAgo), 'Medium');
    await postTask('Overdue yesterday', formatDate(yesterday), 'High'); // High so suggestion panel can suggest

    // Current month: 4 tasks on same day (tomorrow) so "more tasks" indicator (+N more) shows
  ...
  test('should switch back to month view from year view', async ({ page }) => {
    ...
    test('should display tasks on calendar days with due dates', async ({ page }) => {
        // Calendar shows task names on the day cell. Use a due date in current month so it's visible when calendar opens.
        ...
        // Only match current-month day cell (exclude .other-month)
  ...
  test('should handle navigation between months correctly', async ({ page }) => {
      ...
      // Prev month: header must return to initial
